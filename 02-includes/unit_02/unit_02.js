...
// Функция считывает значение из i-1 и с помощью includes ищет данный элемент в массиве a1. Выводит в out-1 результат работы метода.
...
// Сравните данную задачу с задачей 2 из юнита 1. Подумайте какой из методов: includes, indexOf более удобен для данной задачи.
// Как тестируется - ввожу числа и проверяю результат.
...
// Функция считывает значение из i-3 и с помощью includes 
// ищет данный элемент во вложенных массивах массива a3. Выводит в out-3  false, если такого элемента нет и true если есть. Обратите внимание! Только числа!
...
// Функция считывает значение из i-5-1 и индекс с которого начинается поиск в массиве с i-5-2 и с помощью includes  ищет данный элемент в массиве a5 c позиции указанной в i-5-2. 
...
const f5 = () => {
    let inputElement = +document.querySelector('.i-5-1').value;
    let inputIndex = +document.querySelector('.i-5-2').value;
    let out = document.querySelector('.out-5');
    if (a5.includes(inputElement, inputIndex)) {
        out.textContent = 'true';
    } else {
        out.textContent = 'false';
...
const f6 = () => {
    ...
    let a6String = a6.join(' ');

    if (a6String.includes(input)) {
        out.textContent = 'true';
    } else {
        out.textContent = 'false';
    }
...
const f7 = (arr, elem) => {
    ...
    let found = false;
    console.log("Искомое число:", elem);
    for (let i = 0; i < arr.length; i++) {
        console.log("Проверяемый элемент:", arr[i]);
        if (arr[i] === elem) {
            found = true;
            break;
        }
...
document.querySelector('.b-7').addEventListener('click', () => f7(a7, 233));

// TASK 08
// По нажатию b-8 выполняется функция f8. Функция должна получить символ из i-8 и с помощью includes проверить есть ли подобный символ в строке a8 или нет. 
...
const f9 = () => {
    ...
    if (!isNaN(numInput) && a9.includes(numInput)) {
        isFound = true;
    } else {
        // Проверка символов в нижнем и верхнем регистрах
        const lowerCaseInput = input.toLowerCase();
        const upperCaseInput = input.toUpperCase();
        if (a9.includes(lowerCaseInput) || a9.includes(upperCaseInput)) {
            isFound = true;
        }
    }
    out.textContent = isFound ? 'true' : 'false';
}

document.querySelector('.b-9').addEventListener('click', f9);

// TASK 10
// Ну и на прокачку ваших скиллов. Часто попадется вопрос, в чем отличие includes от indexOf? 
// Ответ - в поведении с NaN. Изучите и запомните пример ниже.
...
// В этом примере задача демонстрирует, что метод includes в JavaScript способен корректно определять присутствие значения NaN в массиве, тогда как indexOf этого сделать не может.
